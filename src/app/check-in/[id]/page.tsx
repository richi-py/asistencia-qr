    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error('Esta clase ya no existe o el código es inválido.');
        }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="glass p-8 rounded-2xl max-w-md w-full relative z-10 shadow-2xl border border-slate-700/50">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">Ingresa tus datos para registrar tu presencia en la clase.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {error && (
            <p className="text-red-400 text-sm animate-pulse bg-red-400/10 border border-red-400/20 p-3 rounded-lg">{error}</p>
          )}
